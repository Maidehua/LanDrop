...
class FileServer(private val context:Context) {
    ...
    fun start(){ val dir=File(context.getExternalFilesDir(null),"Download/LanDrop").apply{mkdirs()}; engine=embeddedServer(CIO,port=Protocol.HTTP_PORT,host="0.0.0.0") { routing {
        ...
        post("/api/files"){ var saved:File?=null; var size=0L; call.receiveMultipart().forEachPart{part-> if(part is PartData.FileItem){ val clean=sanitize(part.originalFileName?:"file.bin"); val out=unique(dir,clean); part.provider().toInputStream().use{input->out.outputStream().use{output->size=input.copyTo(output)}}; saved=out }; part.dispose() }; if(saved==null)call.respondText("{\"ok\":false,\"error\":\"missing file\"}",ContentType.Application.Json,HttpStatusCode.BadRequest) else call.respondText(buildJsonObject{put("ok",true);put("name",saved!!.name);put("size",size)}.toString(),ContentType.Application.Json,HttpStatusCode.Created) }
    }}.start(wait=false) }
